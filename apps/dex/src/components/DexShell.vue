<template>
  <main class="min-h-screen overflow-hidden bg-night text-slate-100">
    <div
    ...
    <section
      class="relative mx-auto flex w-full max-w-7xl flex-col gap-5 px-4 py-4 sm:px-6 lg:px-8"
    >
      <header
...
const cluster = ref<ClusterName>(DEFAULT_CLUSTER);
...
const minimumReceivedText = computed(() =>
  quoteState.value.quote && outputToken.value
    ? `${formatTokenAmount(quoteState.value.quote.minimumOutputAmount, outputToken.value.decimals, 6)} ${outputToken.value.symbol}`
    : "--",
);
  ...
  await prepareAction("createPool", async () => {
    ...
    return {
      ...
      rows: [
        {
          label: "Token A",
          value: `${formatTokenAmount(amountA, mintA.decimals)} ${mintA.symbol}`,
        },
        {
          label: "Token B",
          value: `${formatTokenAmount(amountB, mintB.decimals)} ${mintB.symbol}`,
        },
      ...
      h(
        ...
        selectProps.pools.map((pool) =>
          h(
            ...
            `${pool.sides[0].symbol}/${pool.sides[1].symbol} · ${shortAddress(pool.address, 5)}`,
          ),
        ),
      );
  },
        ...
        [
          ...
          h("div", { class: "mt-3 grid gap-2 text-sm sm:grid-cols-4" }, [
            ...
            h(
              "span",
              { class: "rounded-md bg-white/5 px-2 py-1" },
              `Wallet ${formatTokenAmount(rowProps.pool.walletLpBalance ?? 0n, 9, 4)}`,
            ),
          ]),
        ],
      );
