...
import { deriveTokenMetadataPda } from "./pdas";
...
describe("cp-swap client helpers", () => {
  it("builds initialize accounts for the deployed smoke pair", () => {
    ...
    expect(deriveTokenMetadataPda(accounts.lpMint).toString()).toBe(
      "EAeybAXrm5UYheGGihyqPRMVXtJVNA89ExPpSKzxmS11"
    );
    expect(sortedTokenAmounts(input).token0Amount).toBe(200n);
    expect(sortedTokenAmounts(input).token1Amount).toBe(100n);
  });

  it("builds LP metadata within Metaplex byte limits", () => {
    expect(
      buildLpMetadata({ token0Symbol: "DMIM", token1Symbol: "SMOKE" })
    ).toEqual({
    ...
    const long = buildLpMetadata({
      token0Symbol: "VERYLONGTOKEN",
      token1Symbol: "ANOTHERLONG",
    });
    expect(long.name).toBe("MIM Swap LP: VERYLONGTOKEN/ANOTH");
    expect(long.symbol).toBe("VERY-ANOTH");
    expect(new TextEncoder().encode(long.name).length).toBeLessThanOrEqual(32);
    expect(new TextEncoder().encode(long.symbol).length).toBeLessThanOrEqual(
      10
    );
  });
