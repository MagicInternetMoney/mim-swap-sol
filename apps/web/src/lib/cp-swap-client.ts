...
function metadataSymbolPart(value: string): string {
  const normalized = value.trim().replace(/\s+/g, "");
  return normalized || "TOKEN";
}

function buildLpMetadataSymbol(token0Symbol: string, token1Symbol: string) {
...
export function buildLpMetadata(input: {
  token0Symbol: string;
  token1Symbol: string;
  uri?: string;
}): LpMetadata {
  const token0 = metadataSymbolPart(input.token0Symbol);
  const token1 = metadataSymbolPart(input.token1Symbol);
  const pairName = `${token0}/${token1}`;
  const nameBudget =
    LP_METADATA_MAX_NAME_BYTES - utf8Bytes(LP_METADATA_NAME_PREFIX);
  const name = `${LP_METADATA_NAME_PREFIX}${truncateUtf8(
    pairName,
    nameBudget
  )}`;
  const uri = input.uri ?? "";
...
export async function buildInitializePoolTransaction(
  ...
  const token1Amount = accounts.token1Mint.equals(input.tokenAMint)
    ? input.tokenAAmount
    : input.tokenBAmount;
  const token0Symbol = accounts.token0Mint.equals(input.tokenAMint)
    ? input.tokenASymbol
    : input.tokenBSymbol;
  const token1Symbol = accounts.token1Mint.equals(input.tokenAMint)
    ? input.tokenASymbol
    : input.tokenBSymbol;
  const lpMetadata = buildLpMetadata({
    token0Symbol:
      token0Symbol ?? shortAddress(accounts.token0Mint.toString(), 5),
    token1Symbol:
      token1Symbol ?? shortAddress(accounts.token1Mint.toString(), 5),
    uri: input.lpMetadataUri,
  });
  ...
  const metadataIx = await program.methods
    ...
    .accounts({
      ...
      metadata: deriveTokenMetadataPda(accounts.lpMint),
    })
    .instruction();

  return transaction.add(initializeIx, metadataIx);
}
