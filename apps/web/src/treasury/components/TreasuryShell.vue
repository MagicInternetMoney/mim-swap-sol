<template>
  <main
    class="mim-dark-surface text-slate-100"
    :class="embedded ? 'min-h-0 bg-transparent' : 'mim-shell'"
  >
    <section
    ...
    >
      ...
      <section
        v-if="props.walletErrorMessage && !embedded"
      ...
      <section
        v-if="showTreasuryMode || showFeeOpsMode"
      ...
      <section
        v-if="showFeeOpsMode && isAdminWallet"
        class="rounded-lg border border-black/10 bg-white p-4 shadow-sm"
      ...
      <section
        v-else-if="showFeeOpsMode"
        class="rounded-lg border border-black/10 bg-white p-6 text-sm font-semibold text-black/55 shadow-sm"
      >
        Connect the treasury authority wallet to manage fee sweeping and
        conversion.
      </section>
      ...
      >
        ...
        <div v-if="addressRows.length" class="grid gap-2">
          <div
...
type TreasuryMode = "stake" | "treasury" | "feeOps" | "about";
type Confirmation = {
  kind: ActionKind;
  title: string;
  clusterLabel: string;
...
const displayMode = computed(() => props.mode ?? "treasury");
const showStakeMode = computed(() => displayMode.value === "stake");
const showTreasuryMode = computed(() => displayMode.value === "treasury");
const showFeeOpsMode = computed(() => displayMode.value === "feeOps");
const showAboutMode = computed(() => displayMode.value === "about");
const clusterOptions = CLUSTERS;
const cluster = ref<ClusterName>(props.clusterName ?? initialCluster());
