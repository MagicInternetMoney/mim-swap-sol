...
describe("solana config", () => {
  ...
  it("defines both token program variants", () => {
    expect(TOKEN_PROGRAMS.splToken).toBe(
      "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA"
    );
    expect(TOKEN_PROGRAMS.token2022).toBe(
  ...
  it("matches devnet treasury PDA derivations", () => {
    ...
    expect(devnet.treasury.reserveTokenProgram).toBe(TOKEN_PROGRAMS.splToken);
